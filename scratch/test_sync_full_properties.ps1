...
$logFile = "C:\RAMS\sync_full_results.txt"
$sb = New-Object System.Text.StringBuilder

[void]$sb.AppendLine("=== STARTING SYNC FULL PROPERTIES TEST ===")
...
# --- PROCESS 2: WRITE TO OFFICE 2 WITH FULL PROPERTIES ---
$writeCmd = {
    $ErrorActionPreference = "Stop"
    Set-Location "D:\RIMS"
    ...
    foreach ($t in $templates) {
        ...
        # Populate all other properties to match master device
        $enroll.AccessControl = $t.AccessControl
        $enroll.AttType = 0
        $enroll.Department = 0
        ...
        $enroll.UserExtInfo = ""
        $enroll.UserGroup = 0
        $enroll.UseUserGroupACTZ = $false
        $enroll.UseUserGroupVM = $false
        $enroll.ValidDate = [DateTime]$t.ValidDate
        $enroll.ValidityPeriod = $false
