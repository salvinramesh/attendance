...
$logFile = "C:\RAMS\conversion_end_to_end_results.txt"
$sb = New-Object System.Text.StringBuilder

[void]$sb.AppendLine("=== STARTING END-TO-END CONVERSION TEST ===")
...
$cmd = {
    ...
    # Get short template from RAMS DB for DIN 27 (Salvin Ramesh)
    ...
    Write-Output "ConvertToLongMessage success! Long bytes length: $($longBytes.Length)"

    # Convert to long fingerprint string
    $longFpStr = $device.MakeLongeFingerprint($longBytes)
    Write-Output "MakeLongeFingerprint success! String length: $($longFpStr.Length)"
    ...
    if ($longFpStr.Length -gt 0) {
        ...
        Write-Error "Generated long fingerprint string is empty!"
    }
}
