...
$logFile = "C:\RAMS\sync_long_le_results.txt"
$sb = New-Object System.Text.StringBuilder

[void]$sb.AppendLine("=== STARTING MAKE LONG FP LE TEST ===")
...
$writeCmd = {
    ...
    # Parse Little-Endian (every 8-char chunk is a 32-bit uint)
    ...
    Write-Output "LE bytes length: $($bytesLE.Length)"

    # Convert to Long Fingerprint string
    $longFpStr = $device.MakeLongeFingerprint($bytesLE)
    Write-Output "Generated Long Fingerprint. Length: $($longFpStr.Length)"

    if ($longFpStr.Length -gt 0) {
        Write-Output "First 150 chars: $($longFpStr.Substring(0, [System.Math]::Min(150, $longFpStr.Length)))"
    }
